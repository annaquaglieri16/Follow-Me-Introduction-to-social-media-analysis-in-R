...
img <- readPNG(file.path("Twitter_tutorial_figures/crown_wordcloud.png"))
grid.raster(img)
```
...
img <- readPNG(file.path("Twitter_tutorial_figures/win.png"))
grid.raster(img)
```
...
img <- readPNG(file.path("Twitter_tutorial_figures/kensington.png"))
grid.raster(img)
```
...
img <- readPNG(file.path("Twitter_tutorial_figures/lucy_sempey.png"))
grid.raster(img)
```
...
```{r interactive_time_series,fig.width=15,fig.height=10,eval=TRUE}
