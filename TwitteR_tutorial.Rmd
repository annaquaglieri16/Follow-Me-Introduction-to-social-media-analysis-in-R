...
```{r first_search, warning=FALSE, message=FALSE}
...
```{r second_search, warning=FALSE, message=FALSE}
...
img <- readPNG(file.path("Figures_Twitter_Tutorial/reproducible.png"))
grid.raster(img)
```
...
img <- readPNG(file.path("Figures_Twitter_Tutorial/cronjob.png"))
grid.raster(img)
```
...
There is a lot of useless text in here `r emo::ji('scream')` !!
...
```{r crown, echo=FALSE, fig.height=5}
img <- readPNG(file.path("Figures_Twitter_Tutorial/crown_wordcloud.png"))
grid.raster(img)
```
...
```{r win, echo=FALSE, fig.height=5}
img <- readPNG(file.path("Figures_Twitter_Tutorial/win.png"))
grid.raster(img)
```
...
```{r kensington, echo=FALSE, fig.height=2}
img <- readPNG(file.path("Figures_Twitter_Tutorial/kensington.png"))
grid.raster(img)
```
...
```{r lucy, echo=FALSE, fig.height=5}
img <- readPNG(file.path("Figures_Twitter_Tutorial/lucy_sempey.png"))
grid.raster(img)
```
...
```{r interactive_time_series,fig.width=15,fig.height=10}
